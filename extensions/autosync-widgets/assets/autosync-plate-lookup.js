...
function aH(r,v){try{var h=gH();h=h.filter(function(x){return x.r!==r;});h.unshift({r:r,mk:v.make||'',md:v.model||''});if(h.length>6)h=h.slice(0,6);localStorage.setItem(HK,JSON.stringify(h));}catch(e){}}
function sHist(){if(!HD)return;var h=gH();if(!h.length){HD.style.display='none';return;}while(HD.firstChild)HD.removeChild(HD.firstChild);var lb=el('span','apl-history__label','Recent:');HD.appendChild(lb);for(var i=0;i<h.length;i++){(function(x,idx){var it=el('div','apl-history__item');var btn=el('button','apl-history__btn');btn.type='button';btn.appendChild(el('span','apl-history__reg',fR(x.r)));if(x.mk)btn.appendChild(el('span','apl-history__vehicle',x.mk+(x.md?' '+x.md:'')));btn.addEventListener('click',function(){I.value=fR(x.r);lr=x.r;doL(x.r);HD.style.display='none';});it.appendChild(btn);var rm=el('button','apl-history__remove');rm.type='button';rm.title='Remove';rm.innerHTML='<svg width="10" height="10" viewBox="0 0 16 16" fill="none"><path d="M4 4l8 8M12 4l-8 8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>';rm.addEventListener('click',function(e){e.stopPropagation();var hist=gH();hist.splice(idx,1);try{localStorage.setItem(HK,JSON.stringify(hist));}catch(err){}sHist();});it.appendChild(rm);HD.appendChild(it);})(h[i],i);}HD.style.display='flex';}
