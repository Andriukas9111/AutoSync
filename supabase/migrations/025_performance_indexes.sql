...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_shop_fitment_status
  ON products(shop_id, fitment_status);

-- Vehicle page sync status: WHERE shop_id = X AND sync_status = Y
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vehicle_page_sync_shop_status
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_shop_synced
  ON products(shop_id) WHERE synced_at IS NOT NULL;

-- Sync jobs by shop + status (for active job polling)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sync_jobs_shop_status
  ON sync_jobs(shop_id, status);

-- Collection mappings by shop + type (for model collection count)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_collection_mappings_shop_type
