...
// Deno Edge Function scoring module — extracted from app/routes/app.api.suggest-fitments.tsx
// Removed: Remix action handler (lives in Vercel app for manual-mapping UI)
// Kept: all scoring library functions (buildVehicleProfile, scoreEnginesToSuggestions,
// VARIANT_SUFFIXES, etc.) so the Edge Function can run auto-extraction without
// depending on the Vercel HTTP endpoint. The Vercel-hosted manual-mapping UI
// still imports its helpers from the same source-of-truth in app/routes/
// app.api.suggest-fitments.tsx; when that file changes, copy the diff here.
