/**
 * Backfill Engine Specs from auto-data.net
 *
 * Fetches spec pages for ~59,000+ engines that have autodata_url but are
 * missing displacement_cc and other key fields. Updates both ymme_engines
 * and ymme_vehicle_specs tables.
 *
 * Usage:
 *   node scripts/backfill-engine-specs.mjs
 *   node scripts/backfill-engine-specs.mjs --delay=2000 --batch=25 --limit=500
 *   node scripts/backfill-engine-specs.mjs --dry-run
 *
 * Resumable: skips engines that already have displacement_cc populated.
 */
...
async function getEngineCount() {
  ...
  if (error) {
    console.error("Failed to count engines:", error.message);
    return 0;
  }
  return count ?? 0;
}
...
async function fetchBatch(limit) {
  // Always fetch from the top: previously updated rows no longer match
  // the displacement_cc IS NULL filter, so offset is always 0.
...
async function processEngine(engine) {
  ...
  try {
    ...
    const hasData =
      specs.displacementCc || specs.cylinders || specs.aspiration ||
      specs.engineModelCode || specs.powerKw || specs.fuelTypeDetail;
...
async function main() {
  console.log("=== Engine Specs Backfill ===");
  console.log(
    "Delay: " + DELAY_MS + "ms | " +
    "Batch: " + BATCH_SIZE + " | " +
    "Limit: " + (MAX_ENGINES || "none") + " | " +
    "Dry run: " + DRY_RUN
  );
  console.log("");

  const totalCount = await getEngineCount();
  console.log("Found " + totalCount + " engines with autodata_url but missing displacement_cc");
  ...
  console.log("Will process " + effectiveTotal + " engines\n");

  // Process in batches. Since we filter by displacement_cc IS NULL and we
  // update displacement_cc, we always re-query from top (processed rows
  ...
  while (processed < effectiveTotal) {
    ...
    if (batch.length === 0) {
      console.log("No more engines to process (all have displacement_cc or no autodata_url).");
      break;
    }
